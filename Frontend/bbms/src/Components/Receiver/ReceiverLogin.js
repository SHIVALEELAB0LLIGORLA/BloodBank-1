...
import RLogin from '../Receiver/RLogin.jpg';
...
function ReceiverLogin(){
    ...
    const handleLogin=() =>{
        if (!username) {
         ...
         if (!password) {
            ...
            setErrorMessage('Password must contain a special character and be less than 8 characters long');
            return;
        }
        const data={ 
            Name:username,
        ...
        const url='http://localhost:60698/api/Receiver/Login';
        axios.post(url,data).then((result) =>{
            const loginResult = result.data;
            alert(loginResult);
            navigate(loginResult === "Login successful!" ? "/Receiver" : "/ReceiverLogin");
        }).catch((error)=>
        {
            alert(error); 
        })
    ...
    <Fragment>
        ...
        <h2 className='lh2'>Login Form</h2>
            <div className="imgcontainer">
            <img src={RLogin} alt="Avatar" className="avatar" />
           </div>
        ...
        <div className='password-input'>
          ...
          >
            ...
            <p>New User <a href='ReceiverRegistration'>SignUp</a></p>
        </div>
    </Fragment>
    )
}
